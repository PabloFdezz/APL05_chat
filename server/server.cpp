...
struct User
{
    ...
    int lastMsgId = -1; // ultimo msg_id difundido de este usuario
};
...
static void handleRegistration(SOCKET regSocket, SOCKET dataSocket, std::vector<User> &users)
{
    ...
    if (!receiveDatagram(regSocket, datagram, from))
        return;
    ...
    int dropAckPercent)
{
    ...
    if (!receiveDatagram(dataSocket, datagram, from))
        return;
